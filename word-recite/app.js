...
let state = read(STATE_KEY, { activeBook: 'zoology-final', progress: {}, today: {}, selectedChapter: {} });
...
function wordStatus(id, value) {
  if (value) state.progress[id] = value;
  const day = new Date().toISOString().slice(0, 10);
  state.today[day] = (state.today[day] || 0) + 1;
  save();
  renderHeader();
  toast(value === 'known' ? '记住了，继续保持' : '已加入待巩固');
  next();
}
function todayCount() {
  return state.today[new Date().toISOString().slice(0, 10)] || 0;
}
...
function checkSpell() {
  const word = queue[currentIndex];
  if (!word) return;
  const answer = normalize($('#spellInput').value);
  const expected = normalize(word.term);
  const ok = answer === expected;
  $('#spellFeedback').textContent = ok ? '✓ 拼写正确' : `正确答案：${word.term}`;
  $('#spellFeedback').className = `feedback ${ok ? 'good' : 'bad'}`;
  wordStatus(word.id, ok ? 'known' : 'again');
}
